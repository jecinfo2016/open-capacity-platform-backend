...
//	@Resource
//	private PermitUrlProperties permitUrlProperties; //目前是通过配置文件读取security.oauth2 xiao
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		List<String> blackListIp = blackListService.getBlackListIp();
		if (blackListIp.contains(ip)){
			message.put("resp_code", 401);
			message.put("resp_msg", "ip限制");
...
//			flag = Lists.newArrayList(permitUrlProperties.getIgnored()).stream().anyMatch((item) -> {
			flag = Lists.newArrayList(whiteListService.getIgnore()).stream().anyMatch((item) -> {
				try {
